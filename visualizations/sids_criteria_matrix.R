...
# ── 2. Sovereign SIDS subset ──────────────────────────────────────────────────
# Associate members (Aruba, Curaçao, etc.) are excluded because the criterion
# framework presupposes statehood and produces misleading results otherwise.

sids <- countries |>
  filter(sids_tier == "Sovereign member") |>
...
sids_long <- sids |>
  ...
  mutate(
    ...
    # most exceptions at top of chart
...
col_pass   <- "#749c4c"   # LovelyData green
  col_out    <- "#f38439"   # LovelyData orange
    col_multi  <- "#44759e"   # LovelyData blue — 2+ exception country names
      col_bg     <- "white"
        col_text   <- "#605b54"   # LovelyData gray-brown
          ...
          y_colours <- rep(col_text, length(y_order))
          ...
          p <- ggplot(sids_long, aes(x = criterion, y = label)) +
            ...
            labs(
              ...
              subtitle = paste0(
                "39 officially recognised sovereign Small Island Developing States, tested against each\n",
                "component of the acronym they are assigned: Small (pop. ≤ 1.5M) · Island (insular territory) ·\n",
                "Developing (WB non-high income) · Sovereign (independent state). Orange = outside criterion."
              ),
              caption  = paste0(
            ...
            theme(
              ...
              axis.text.x       = element_markdown(color = col_text, size = 8.5,
